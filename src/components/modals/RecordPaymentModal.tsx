...
export const RecordPaymentModal: React.FC = () => {
  ...
  const [paymentAmount, setPaymentAmount] = useState<number>(khataLedger[0]?.outstandingBalance || 1000);
  const [paymentMode, setPaymentMode] = useState<string>('UPI (PhonePe / GPay)');
  const [receiptNote, setReceiptNote] = useState('Payment settlement');

  const selectedCustomer = khataLedger.find((k) => k.id === selectedCustomerId) || khataLedger[0];

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const custId = selectedCustomerId || selectedCustomer?.id;
    if (!custId || paymentAmount <= 0) return;

    recordKhataPayment(custId, paymentAmount, paymentMode);
      ...
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl border border-[#E2EAE5] overflow-hidden flex flex-col">
        ...
        {khataLedger.length === 0 ? (
          <div className="p-8 text-center text-xs text-[#7A8B82]">
            <p className="mb-4">No active Khata customer credit accounts found in the database.</p>
            <button
              onClick={() => setActiveModal('none')}
          ...
          <form onSubmit={handleSubmit} className="p-4 sm:p-5 flex flex-col gap-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Crop harvest settlement"
              />
            </div>
